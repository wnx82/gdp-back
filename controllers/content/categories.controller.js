...
const create = catchAsync(async (req, res) => {
    ...
    try {
        ...
        console.log(`----------->La categorie a bien été créé<-----------`);
...
const deleteOne = catchAsync(async (req, res) => {
    ...
    if (force === undefined || parseInt(force, 10) === 0) {
        // Vérification si la catégorie a déjà été supprimée de manière logique
        const categorie = await collection.findOne({ _id: new ObjectId(id) });
        if (categorie?.deletedAt) {
        ...
        if (result.deletedCount === 1) {
            ...
            res.status(200).json(success(`🗑️ Suppression d'une catégorie de manière physique`));
            redisClient.del(`${collectionName}:all`);
            redisClient.del(`${collectionName}:${id}`);
        } else {
